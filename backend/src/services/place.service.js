...
const { AuthFailureError, ErrorResponse, NotFoundError, BadRequestError } = require('../helpers/error.response')
...
class PlaceService {
    static nearByList = async (information) => {
        ...
        */
        ...
        listPlaces.forEach(async (place) => {
            ...
            if (ExistPlace) { console.log(`${ExistPlace.name} is already save in database`) }
            if (!ExistPlace) {
                ...
                if (newPlace) { console.log(`added ${newPlace.name} information`) }
            }
        })
    ...
    static changeApprovePlace = async ({ id, isApproved }) => {
